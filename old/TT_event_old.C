...
////////////////////////////////////////////////////////////////////////
void TT_event::Init_vectors(Double_t Enu,Double_t q[4])
{
  f_k_lower[0]=Enu;
  f_k_lower[1]=0.0;
  f_k_lower[2]=0.0;
...
void TT_event::Evaluate_dsigma_dall()
{
  ...
  Bool_t good_event=Setup_kinematics();
  if (!good_event) {
    f_bad_event=kTRUE;
    return;
  }
...
Bool_t TT_event::Setup_kinematics()
{
  ...
  if (f_process==0) result=Single_nucleon_differential();
  if (f_process==1) result=AS_SM_differential();
  if (f_process==2) result=AS_MF_differential();
  if (f_process==3) result=AS_corr_differential();
  if (f_process==4) result=SM_SM_differential();
...
{
  Double_t ans=0.0;
  if (f_process==0) ans=Single_nucleon_evaluate_integrand();
  if (f_process==1) ans=SM_evaluate_integrand();
  if (f_process==2) ans=AS_MF_evaluate_integrand();
  if (f_process==3) ans=AS_corr_evaluate_integrand();
  if (f_process==4) ans=SM_evaluate_integrand();
  
  return ans;
}

////////////////////////////////////////////////////////////////
Bool_t TT_event::AS_SM_differential()
{
  f_dsigma_dall=0.0;
  ...
  if (TMath::Abs(SM_argument_of_delta()>f_params->f_numerical_threshold)) {
    cout << "arg: " << SM_argument_of_delta() << endl;
    return kFALSE;
  }
  ...
  Bool_t blocked=sqrt(f_pprime_lower[0]*f_pprime_lower[0]-f_params->f_M_recoil*f_params->f_M_recoil)<f_nucleus->f_SM_p_fermi;

  if (f_params->f_do_Pauli_blocking && blocked) return kFALSE;
  else return kTRUE;
}

////////////////////////////////////////////////////////////////
Bool_t TT_event::SM_SM_differential()
{
  f_dsigma_dall=0.0;

  Double_t sinth_q=sqrt(1.0-f_cos_theta_q*f_cos_theta_q);
  f_p_lower[0]=sqrt(f_mag_p*f_mag_p+f_params->f_M_target*f_params->f_M_target)-f_nucleus->f_SM_e_bind;
...
////////////////////////////////////////////////////////////////
Bool_t TT_event::AS_MF_differential()
{
  f_dsigma_dall=0.0;
  ...
  if (TMath::Abs(AS_MF_argument_of_delta()>f_params->f_numerical_threshold)) {
    cout << "arg: " << AS_MF_argument_of_delta() << endl;
    return kFALSE;
  }
  if (TMath::Abs(f_cos_theta_pq)>1.0) return kFALSE;
  Double_t sinth_pq=sqrt(1.0-f_cos_theta_pq*f_cos_theta_pq);
  ...
  f_pprime_lower[0]=sqrt(f_pprime_lower[0]);
  Bool_t blocked=sqrt(f_pprime_lower[0]*f_pprime_lower[0]-f_params->f_M_recoil*f_params->f_M_recoil)<f_nucleus->f_AS_SF_p_fermi;
...
////////////////////////////////////////////////////////////////
Bool_t TT_event::AS_corr_differential()
{
  f_dsigma_dall=0.0;
...
//////////////////////////////////////////////////////////////////
Bool_t TT_event::Single_nucleon_differential()
{
  f_dsigma_dall=0.0;

  f_p_lower[0]=f_params->f_M_target;
...
////////////////////////////////////////////////////////////////
Double_t TT_event::Single_nucleon_evaluate_integrand()
{
  return 1.0/(f_p_lower[0]*f_mag_q);
}

////////////////////////////////////////////////////////////////
Double_t TT_event::SM_evaluate_integrand()
{
  Double_t SF_norm=f_nucleus->n_sm(f_mag_p);
  Double_t ans=1.0*SF_norm/(f_p_lower[0]*f_pprime_lower[0]);
  ...
  Double_t denom=TMath::Abs(SM_argument_of_delta_prime());
  if (denom==0.0) cout << "vanishing denominator a" << endl;
  return ans*f_mag_p*f_mag_p/denom;
}

////////////////////////////////////////////////////////////////
Double_t TT_event::AS_MF_evaluate_integrand()
{
  Double_t SF_norm=f_nucleus->n_mf(f_mag_p);
  Double_t ans=1.0*SF_norm/(f_p_lower[0]*f_pprime_lower[0]);
  ...
  Double_t denom=TMath::Abs(AS_MF_argument_of_delta_prime());
  if (denom==0.0) cout << "vanishing denominator a" << endl;
  return ans*f_mag_p*f_mag_p/denom;
}

////////////////////////////////////////////////////////////////
Double_t TT_event::AS_corr_evaluate_integrand()
{
  Double_t SF_norm=f_nucleus->P_corr(f_mag_p,f_E);
  Double_t ans=1.0*SF_norm/(f_p_lower[0]*f_pprime_lower[0]);
  
  //--we converted to spherical coordinates for the initial momentum
  return ans*f_mag_p*f_mag_p;
}
