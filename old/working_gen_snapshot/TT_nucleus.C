...
Double_t TT_nucleus::n_sm(Double_t p)
{
  ...
  Double_t ans=3.0/pow(f_SM_p_fermi,3.0);
  return ans*(f_N+f_Z)/(8.0*TMath::Pi());
}
...
void TT_nucleus::n_plot()
{
  ...
  TFile f("~elaird/ccqe/xs_ted/p_dists.root","RECREATE");
  sm_graph.Write();
  mf_graph.Write();
  corr_graph.Write();
  tot_graph.Write();
...
////////////////////////////////////////////////////////////////////////
Double_t TT_nucleus::P_corr(Double_t p[3],Double_t E)
{
  Double_t p_mag=0.0;
  for (Int_t iComp=0;iComp<2;iComp++) p_mag+=p[iComp]*p[iComp];
  p_mag=sqrt(p_mag);
  if (p_mag==0) {
