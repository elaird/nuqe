void gen_look(Int_t mode) {
  ...
  if (mode==30) {
    TH1D *hv =(TH1D*)f.Get("volume_histo")->Clone();
    TH1D *hvc=(TH1D*)f.Get("volume_check_histo")->Clone();
    f.Close();
  ...
  if (mode==50) {
    ...
    tree->Draw("q_bold**2-w**2>>h","(1.0/inv_weight)","goff");
  ...
  if (mode==60) {
    ...
    gROOT->cd();
    TTree *tree=(TTree*)f.Get("tree")->Clone();
    TGraph *gr_pol=(TGraph*)f_polish.Get("polish_e_sm_pb")->Clone();
    ...
    tree->Draw("0.8-w>>h","(1.0/inv_weight)","goff");
    h.Scale(7.287e-38/h.Integral("width"));
