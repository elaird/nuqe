...
class TT_nucleus {
 public :
  ...
  Double_t P_corr(Double_t p[3],Double_t E);
  void n_plot();
};
...
TT_nucleus::TT_nucleus(Int_t Z, Int_t N)
{

  //this is a 'fake' nucleus for code testing
  if (Z==0 && N==0) {
    f_M_target_nucleus=f_M_n;
    f_M_residual_nucleus=0.0;

    f_AS_SF_p_fermi=0.225;
    f_SM_p_fermi=0.003;
    f_SM_e_bind=0.0;
    f_N=1;
    f_Z=0;
  ...
  if (Z==8 && N==8) {
    ...
    f_avg_p2=0.1622;//actually compute this
  }
