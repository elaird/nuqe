...
void TT_surface::Init_surface()
{
  ...
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
	...
	Double_t cth_p=(i_cth_p+0.5)*(f_cth_p_max-f_cth_p_min)/f_N_cth_p + f_cth_p_min;

	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
	  Double_t phi_p=(i_phi_p+0.5)*(f_phi_p_max-f_phi_p_min)/f_N_phi_p + f_phi_p_min;

	  f_surface[i_w][i_qbold][i_cth_p][i_phi_p]=0.0;	  

	  TV_class foo;
	  foo.Init(f_params,f_nucleus,f_Enu,w,qbold,cth_p,phi_p);
	  foo.Evaluate_d4sigma_dw_dq_bold_domega_p();
	  if (foo.f_d4sigma_dw_dq_bold_domega_p>0.0) {
	    f_surface[i_w][i_qbold][i_cth_p][i_phi_p]=foo.f_d4sigma_dw_dq_bold_domega_p;
	  }
	  //cout << "index=" << Index(i_w,i_qbold,i_cth_p,i_phi_p) << "; xs=" << foo.f_d4sigma_dw_dq_bold_domega_p << endl;
...
Double_t TT_surface::Count_surface_zeroes()
{
  ...
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
    for (Int_t i_qbold=0;i_qbold<f_N_qbold;i_qbold++) {
      for (Int_t i_cth_p=0;i_cth_p<f_N_cth_p;i_cth_p++) {
	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
	  if (f_surface[i_w][i_qbold][i_cth_p][i_phi_p]==0.0) zeroes++;
	  //else cout << f_surface[i_w][i_qbold][i_cth_p][i_phi_p] << ", z:" << zeroes << endl;
	}
      }
    }
  }

  return (zeroes+0.0)/(f_N_w*f_N_qbold*f_N_cth_p*f_N_phi_p);
}

////////////////////////////////////////////////////////////////////////
Double_t TT_surface::Get_volume_factor(Int_t i_w,Int_t i_qbold,Int_t i_cth_p,Int_t i_phi_p)
{
  Double_t ws=(i_w)    *(f_w_max-f_w_min)/f_N_w + f_w_min;
  Double_t p_lep=sqrt(pow(f_Enu-ws,2)-pow(f_params->f_m_lep,2));
  Double_t ws_qbold_min=f_Enu-p_lep;
  ...
  factor*=(f_cth_p_max-f_cth_p_min)/f_N_var;
...
void TT_surface::Compute_volume_and_integral()
{
  ...
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
	...
	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
	  ...
	  Int_t index=Index(i_w,i_qbold,i_cth_p,i_phi_p);
	  f_integral[index]=previous_value;

	  Double_t factor=Get_volume_factor(i_w,i_qbold,i_cth_p,i_phi_p);
	  f_total_volume+=factor;

	  previous_value=f_integral[index]+factor*f_surface[i_w][i_qbold][i_cth_p][i_phi_p];
	  
	  f_integral_histo->SetBinContent(index+1,factor*f_surface[i_w][i_qbold][i_cth_p][i_phi_p]);
...
void TT_surface::Adjust_surface_with_neighbors()
{
  ...
  //ugh--copy surface
  Double_t ini_surface[f_N_w][f_N_qbold][f_N_cth_p][f_N_phi_p];
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
    for (Int_t i_qbold=0;i_qbold<f_N_qbold;i_qbold++) {
      for (Int_t i_cth_p=0;i_cth_p<f_N_cth_p;i_cth_p++) {
	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
	  ini_surface[i_w][i_qbold][i_cth_p][i_phi_p]=f_surface[i_w][i_qbold][i_cth_p][i_phi_p];
	}
      }
    }
  }
  ...
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
	...
	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
	  ...
	  else                        neighbors[0][0]=ini_surface[i_w-1][i_qbold][i_cth_p][i_phi_p];
	  if (i_w==f_N_w-1)           neighbors[0][1]=0.0;
	  else                        neighbors[0][1]=ini_surface[i_w+1][i_qbold][i_cth_p][i_phi_p];
	  if (i_qbold==0         )    neighbors[1][0]=0.0;
	  else                        neighbors[1][0]=ini_surface[i_w][i_qbold-1][i_cth_p][i_phi_p];
	  if (i_qbold==f_N_qbold-1)   neighbors[1][1]=0.0;
	  else                        neighbors[1][1]=ini_surface[i_w][i_qbold+1][i_cth_p][i_phi_p];
	  if (i_cth_p==0      )       neighbors[2][0]=0.0;
	  else                        neighbors[2][0]=ini_surface[i_w][i_qbold][i_cth_p-1][i_phi_p];
	  if (i_cth_p==f_N_cth_p-1)   neighbors[2][1]=0.0;
	  else                        neighbors[2][1]=ini_surface[i_w][i_qbold][i_cth_p+1][i_phi_p];
	  if (i_phi_p==0      )       neighbors[3][0]=0.0;
	  else                        neighbors[3][0]=ini_surface[i_w][i_qbold][i_cth_p][i_phi_p-1];
	  if (i_phi_p==f_N_phi_p-1)   neighbors[3][1]=0.0;
	  else                        neighbors[3][1]=ini_surface[i_w][i_qbold][i_cth_p][i_phi_p+1];

	  Double_t max=ini_surface[i_w][i_qbold][i_cth_p][i_phi_p];
	  ...
	  f_surface[i_w][i_qbold][i_cth_p][i_phi_p]=max;
	}
      }
    }
  }
  ...
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
	...
	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
	  ...
	  for (Int_t side=0;side<3;side++) {
	    ...
	    potential_index=i_cth_p-1+side;
	    ni[2][side]=i_cth_p;
	    if (potential_index>=0 && potential_index<f_N_cth_p) ni[2][side]=potential_index;
...
void TT_surface::Replace_low_bins()
{
  ...
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
    for (Int_t i_qbold=0;i_qbold<f_N_qbold;i_qbold++) {
      for (Int_t i_cth_p=0;i_cth_p<f_N_cth_p;i_cth_p++) {
  	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
  	  if (f_surface[i_w][i_qbold][i_cth_p][i_phi_p]<content) {
  	    f_surface[i_w][i_qbold][i_cth_p][i_phi_p]=content;
  	  }
  	}
      }
    }
...
Double_t TT_surface::Test_surface(Int_t factor)
{
  ...
  for (Int_t i_w=0;i_w<factor*f_N_w;i_w++) {
    ...
    for (Int_t i_qbold=0;i_qbold<factor*f_N_qbold;i_qbold++) {
      ...
      for (Int_t i_cth_p=0;i_cth_p<factor*f_N_cth_p;i_cth_p++) {
	Double_t cth_p=(i_cth_p+0.5)*(f_cth_p_max-f_cth_p_min)/(factor*f_N_cth_p) + f_cth_p_min;
	...
	for (Int_t i_phi_p=0;i_phi_p<factor*f_N_phi_p;i_phi_p++) {
	  ...
	  Int_t ind_cth_p=i_cth_p/factor;
	  Int_t ind_phi_p=i_phi_p/factor;

	  if (foo.f_d4sigma_dw_dq_bold_domega_p>f_surface[ind_w][ind_qbold][ind_cth_p][ind_phi_p]) {
	    //cout << "f_s=" << f_surface[ind_w][ind_qbold][ind_cth_p][ind_phi_p] << "; foo=" << foo.f_d4sigma_dw_dq_bold_domega_p << endl;
	    problems++;
	  }
...
Double_t TT_surface::Test_surface_randomly(Int_t factor)
{
  ...
  for (Int_t i=0;i<max;i++) {
    ...
    Int_t ind_cth_p=f_rand.Integer(f_N_var);
    ...
    Double_t cth_p_min=f_cth_p_min+ ind_cth_p   *(f_cth_p_max-f_cth_p_min)/f_N_cth_p;
    Double_t cth_p_max=f_cth_p_min+(ind_cth_p+1)*(f_cth_p_max-f_cth_p_min)/f_N_cth_p;
    ...
    Double_t cth_p=f_rand.Uniform(cth_p_min,cth_p_max);
    ...
    if (foo.f_d4sigma_dw_dq_bold_domega_p>f_surface[ind_w][ind_qbold][ind_cth_p][ind_phi_p]) {
      printf("random test: w,q,c,p=%d%d%d%d; surf=%8.6g; xs=%8.6g\n",ind_w,ind_qbold,ind_cth_p,ind_phi_p,f_surface[ind_w][ind_qbold][ind_cth_p][ind_phi_p],foo.f_d4sigma_dw_dq_bold_domega_p);
      f_surface[ind_w][ind_qbold][ind_cth_p][ind_phi_p]=foo.f_d4sigma_dw_dq_bold_domega_p;
      problems++;
    }
...
Double_t TT_surface::Fix_surface(Int_t factor)
{
  ...
  for (Int_t i_w=0;i_w<factor*f_N_w;i_w++) {
	...
	for (Int_t i_phi_p=0;i_phi_p<factor*f_N_phi_p;i_phi_p++) {
	  ...
	  if (foo.f_d4sigma_dw_dq_bold_domega_p>f_surface[ind_w][ind_qbold][ind_cth_p][ind_phi_p]) {
	    f_surface[ind_w][ind_qbold][ind_cth_p][ind_phi_p]=2.0*foo.f_d4sigma_dw_dq_bold_domega_p;
	    problems++;
	  }
...
void TT_surface::Scale_surface(Double_t scale_factor)
{
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
    for (Int_t i_qbold=0;i_qbold<f_N_qbold;i_qbold++) {
      for (Int_t i_cth_p=0;i_cth_p<f_N_cth_p;i_cth_p++) {
	for (Int_t i_phi_p=0;i_phi_p<f_N_phi_p;i_phi_p++) {
	  f_surface[i_w][i_qbold][i_cth_p][i_phi_p]*=scale_factor;
	  Int_t index=Index(i_w,i_qbold,i_cth_p,i_phi_p);
...
////////////////////////////////////////////////////////////////////////
Int_t TT_surface::Index(Int_t i_w,Int_t i_qbold,Int_t i_cth_p,Int_t i_phi_p)
{
  Int_t ans=0;
...
////////////////////////////////////////////////////////////////////////
Bool_t TT_surface::Draw_point(TV_class *event,Double_t& inv_weight)
{
  ...
  Int_t i_cth_p=(index/f_N_var)%f_N_var;
  index-=f_N_var*i_cth_p;
  ...
  if (index_orig!=Index(i_w,i_qbold,i_cth_p,i_phi_p)) printf("uh oh: index=%d; (i_w;i_qbold;i_cth_p;i_phi_p)=(%d;%d;%d;%d)\n",index_orig,i_w,i_qbold,i_cth_p,i_phi_p);
  ...
  Double_t cth_p_min= i_cth_p   *(f_cth_p_max-f_cth_p_min)/f_N_cth_p+f_cth_p_min;
  Double_t cth_p_max=(i_cth_p+1)*(f_cth_p_max-f_cth_p_min)/f_N_cth_p+f_cth_p_min;
  ...
  event->Init(f_params,f_nucleus,f_Enu,w,qbold,cth_p,phi_p);
  event->Evaluate_d4sigma_dw_dq_bold_domega_p();

  //rejection method
  Double_t y=f_rand.Uniform(0.0,f_surface[i_w][i_qbold][i_cth_p][i_phi_p]);

  inv_weight=1.0;
  if (f_surface[i_w][i_qbold][i_cth_p][i_phi_p]<event->f_d4sigma_dw_dq_bold_domega_p) {
    //cout << "broken surface=" << Index(i_w,i_qbold,i_cth_p,i_phi_p) << "," << f_surface[i_w][i_qbold][i_cth_p][i_phi_p]/event->f_d4sigma_dw_dq_bold_domega_p << endl;
    inv_weight=f_surface[i_w][i_qbold][i_cth_p][i_phi_p]/event->f_d4sigma_dw_dq_bold_domega_p;
  }

  if (y<event->f_d4sigma_dw_dq_bold_domega_p) return kTRUE;
  else return kFALSE;
}
