...
class TT_surface {
 public :
  ...
  //Double_t f_surface[f_N_w][f_N_qbold][f_N_mag_p][f_N_phi_p];
  //Double_t f_integral[f_N_w*f_N_qbold*f_N_mag_p*f_N_phi_p];
  Double_t ****f_surface;
  Double_t *f_integral;
  ...
  virtual ~TT_surface();
  void    Init_surface();
  Double_t Get_volume_factor(Int_t i_w,Int_t i_qbold,Int_t i_mag_p,Int_t i_phi_p);
...
TT_surface::TT_surface(Double_t Enu,TT_params *params,TT_nucleus *nucleus,Int_t N_dims,Int_t N_var)
{
  ...
  f_integral=new Double_t[f_N_w*f_N_qbold*f_N_mag_p*f_N_phi_p];
  ...
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
    ...
    for (Int_t i_qbold=0;i_qbold<f_N_qbold;i_qbold++) {
      f_surface[i_w][i_qbold]=new Double_t*[f_N_mag_p];
      for (Int_t i_mag_p=0;i_mag_p<f_N_mag_p;i_mag_p++) {
	f_surface[i_w][i_qbold][i_mag_p]=new Double_t[f_N_phi_p];
      }
    }
  }
  ...
  f_mag_p_min=0.0;
  if (f_params->f_do_Smith_Moniz) f_mag_p_max=f_nucleus->f_SM_p_fermi;
  if (f_params->f_do_AS_MF) f_mag_p_max=f_nucleus->f_AS_MF_gen_p_max;
...
TT_surface::~TT_surface() {
  for (Int_t i_w=0;i_w<f_N_w;i_w++) {
    for (Int_t i_qbold=0;i_qbold<f_N_qbold;i_qbold++) {
      for (Int_t i_mag_p=0;i_mag_p<f_N_mag_p;i_mag_p++) {
	delete f_surface[i_w][i_qbold][i_mag_p];
      }
