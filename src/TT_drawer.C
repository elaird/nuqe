...
Double_t TT_drawer::Flux_histo_height()
{
  ...
  Double_t Enu_left=histo->GetBinLowEdge(bin);
  Double_t Enu_right=histo->GetBinLowEdge(bin+1);
  Double_t fraction=(Enu-Enu_left)/(Enu_right-Enu_left);

  Double_t height_left=histo->GetBinContent(bin);
  Double_t height_right=histo->GetBinContent(bin+1);
